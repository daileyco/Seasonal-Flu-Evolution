...
# > as.numeric(Sys.time())
# [1] 1711988922
# > as.POSIXct(1711988922)
# [1] "2024-04-01 12:28:42 EDT"
set.seed(1711988922)

## set up loop to 
### 1 resample data for single metric per subtype*season*location (pooled for flu B)
### 2 create space lagged variables
### 3 create space time lagged variables
### 4 subset data to 2015-2020
...
for(i in 1:1000){
  ...
  this.data <- smalltrees%>%
    ...
    mutate(subtype = ifelse(subtype %in% c("BVic", "BYam"), "B", subtype)) %>%
    
    ###filter out missing rows for pooled flu b
    group_by(subtype, season,location) %>% 
    mutate(flag1 = is.na(sum(n)), 
           flag2 = sum(n,na.rm=T)!=0, 
           flag3 = is.na(mpd), 
  ...
  this.data <- this.data %>%
    ...
    filter(season %in% c("2015-2016", "2016-2017", "2017-2018", "2018-2019", "2019-2020"))
  ...
  combos <- expand.grid(x = paste0("mpd_", c("B", "H1", "H3")), 
                        y = names(tdfw)[-2:-1]) %>%
...
## save
save(cordf, file = "./01-Data/02-Analytic-Data/correlations_subset1520.rds")
