...
write.table(c(2e-3),
            file = "./01-Data/01-Processed-Data/alt/IQTREE/rate_a.tsv",
            sep = "\t",
            row.names = F,
            quote = F, 
            col.names = F)
write.table(c(6e-4),
            file = "./01-Data/01-Processed-Data/alt/IQTREE/rate_b.tsv",
            sep = "\t",
            row.names = F,
            quote = F, 
            col.names = F)
  ...
  for(i in start:stop){
    ...
    commands <- c(commands,
                  paste0('"C:/Program Files/iqtree-2.4.0-Windows/bin/iqtree2.exe" -s "',
                         ...
                         ' --date-outlier 3',
                         # ifelse(i%in%c(739), '', ' --date-outlier 3'),
                         # ifelse(i%in%c(665:666, 739), '', ' --date-outlier 3'), 
                         ifelse(metadata$subtype[i]%in%c("H3", "H1"), ' --date-options "-w G:/Research/Seasonal-Flu-Evolution/01-Data/01-Processed-Data/alt/IQTREE/rate_a.tsv"', ' --date-options "-w G:/Research/Seasonal-Flu-Evolution/01-Data/01-Processed-Data/alt/IQTREE/rate_b.tsv"')
                         # ' --date-options "-t 1e-4"'
