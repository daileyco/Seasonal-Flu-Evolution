...
t.list <- lapply(t.list,
  ...
  lapply(., 
         (\(x){
           x %>% filter(!duplicated(.[,c("Variable","Levels", "BVic")]))
         }))
...
caps <- c("Summaries of Full Phylogenetic Trees", "Summaries of Full Phylogenetic Trees, 2015-2020", 
          "Summaries of Co-circulating Lineages / Subtrees extracted from Full Trees", "Summaries of Co-circulating Lineages / Subtrees extracted from Full Trees, 2015-2020")
...
for(i in 1:length(ft.list)){
  ...
  print(temp, 
        target = paste0("./03-Output/01-Tables/table_treesummaries_", 
                        names(ft.list)[i], 
                        ".docx"))
...
## save
save(ft.list, 
     file = "./03-Output/01-Tables/flextables_treesummaries.rds")
